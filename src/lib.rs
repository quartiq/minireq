...
pub struct Minireq<'a, Context, Stack, Clock, Broker, E>
where
    ...
    E: core::fmt::Display,
{
    ...
    fn _handle_mqtt<F>(&mut self, f: F) -> Result<(), Error<Stack::Error>>
    where
        F: Fn(Handler<Context, E>, &str, &[u8], &mut [u8]) -> Result<usize, E>,
    {
        let MinireqContext {
            handlers,
            mqtt,
